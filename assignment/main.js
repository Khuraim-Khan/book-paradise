...
window.onclick = (Event) => {
    if (Event.target == overlay) {
        const modals = doqument.querySelectorAll(".modal");
...
function validate() {
    ...
    // Message should less than 500 characters
    if (message.length >= 150) {
        text = "Please Enter Less Than 500 Characters ";
        error_message.innerHTML = text;
        return false;
    }
    // This alert message will apear if all form fields are filled correctly
...
function sortList() {
    ...
    while (switching) {
        switching = false;
        //loop is running through each products
